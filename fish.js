...
const scaleRowCount = 60; // how many scale rows
const ptsOnScale = 6; // how many points used to build a scale's Polyline

const tailLines = 15; // how many lines used to draw the tail
...
const scaleRelativeDimension = 1.2; // scale "radius" as a proportion of the distance between rows
...
function drawScale(scale) {
  ...
  const points = Array(ptsOnScale).fill(null).map((_, i) => {
    ...
    const pt = rotatePoint(scale.top, angle, scale.center);
    return pt;
  });
...
function drawHead() {
  ...
  const topPts = getPtsOnCurve(5)(curveTop, 1, 1 - headSpace);
  // topPts.shift();
  topPts.pop();
  const btmPts = getPtsOnCurve(5)(curveBtm, 1 - headSpace, 1);
  // btmPts.pop();
...
function drawTail() {
  ...
  const x = topPt.x - ((scaleRowCurvature * (x1 - x2) / 0.5) * ((btmPt.y - topPt.y) / height * curvinessRelativeToLength));
  const curve = new CubicBezier(topPt, { x, y: topPt.y + (scaleRowCurvature * (x2 - x1) * 0.1) }, { x, y: btmPt.y - (scaleRowCurvature * (x2 - x1) * 0.1) }, btmPt, ctx);
  drawFin(curve.points, { min: 100, max: 400 }, { min: Math.PI * (5 / 4), max: Math.PI * (3 / 4) }, midPointMinimumAdjust(0.9), linearAdjust(1));
}
